...
public abstract class Java2CTests implements TestInterface {
     ...
     */
    static final String OUTPUT_LIB_DIR = "build" + File.separator + "libsrc";

    /**
     * Applies the Java2C obfuscation to a class
     *
     * @param pathToFile the path to the file to be transformed (starting from where the package structure starts)
     * @param libName    the name of the output lib. Without prepending the `lib` characters or appending .so, .dll etc..
     * @param destDir    The directory containing the class to be transformed, only up to where the java package structure
     *                   starts
     */
    void transformJava2C(String pathToFile, String libName, String destDir) {
        JavaToC j2c = new JavaToC();
        try //transformation
        {
            j2c.startParsing(OUTPUT_LIB_DIR, libName);
            j2c.parseClass(destDir + "/" + pathToFile);
            j2c.endParsing();
        } catch (IOException e) {
    ...
    /**
     * Build the files generated by Java2C
     *
     * @param libname  the name of the library, same name used in the transformation process
     * @param compiler The compiler that will compile the C code. If null the default one will be used.
     */
    void buildJava2C(String libname, @Nullable NativeCompiler compiler) {
        //building
